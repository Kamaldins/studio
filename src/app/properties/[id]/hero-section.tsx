...
const HeroSection = ({ images, openSlider, miniGalleryIndex, nextMiniGallery, prevMiniGallery }: HeroSectionProps) => {
  ...
  // Calculate visible images - show exactly 6 images
  const getVisibleImages = () => {
    const visibleCount = 6; // Always show exactly 6 images
        ...
        <div className="max-w-6xl mx-auto mb-4 sm:mb-8 px-4 sm:px-0">
          <div className="flex items-center justify-center gap-2 sm:gap-3">
            <button
              ...
              className="flex-shrink-0 bg-card/90 dark:bg-slate-800/90 hover:bg-card dark:hover:bg-slate-700 text-foreground dark:text-slate-200 p-2 sm:p-3 rounded-full shadow-xl transition-all duration-300 hover:scale-110 active:scale-95 border"
            >
              ...
              {visibleImages.map((item, index) => (
                ...
                >
                  <Image
                    ...
                    fill
                    className="object-cover rounded-xl sm:rounded-2xl shadow-lg group-hover:scale-110 transition-transform duration-300 border-2 border-transparent group-hover:border-primary"
                    sizes="15vw"
                  />
